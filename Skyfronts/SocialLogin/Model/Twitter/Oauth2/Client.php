 ...
 * @author		CedCommerce Magento Core Team <Ced_MagentoCoreTeam@cedcommerce.com>
 */
namespace Skyfronts\SocialLogin\Model\Twitter\Oauth2;
...

{
    ...
    const REDIRECT_URI_ROUTE = 'skyfrontssociallogin/twitter/connect';

    const REQUEST_TOKEN_URI_ROUTE = 'skyfrontssociallogin/twitter/request';
    ...
    const XML_PATH_ENABLED = 'customer/skyfronts_sociallogin_twitter/enabled';

    const XML_PATH_CLIENT_ID = 'customer/skyfronts_sociallogin_twitter/client_id';

    const XML_PATH_CLIENT_SECRET = 'customer/skyfronts_sociallogin_twitter/client_secret';
      ...
      //  Mage::log($response->getStatus().' - '. $response->getBody());
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                ...
                throw new Skyfronts_SocialLogin_TwitterOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('sociallogin')
